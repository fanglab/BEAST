...
then
	echo "#############################################################################################################################"
	echo "Searches Prophages using Phage Finder (Fouts et al, 2006)."
	echo ""
	echo "Requires: PhageFinder, Entrez Direct (EDirect)"
	echo ""
	echo "Usage: GO_Prophages.sh <genome_accession_number> <prefix>"
	echo "#############################################################################################################################"
   exit 0
fi
...
chr_length=`awk '/^>/ {if (seqlen) print seqlen;seqlen=0;next} {seqlen+=length($0)}END{print seqlen}' temp0 | uniq`
